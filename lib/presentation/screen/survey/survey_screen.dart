...
class ScreenSurveyState extends State<ScreenSurvey> {
  ...
  void initState() {
    schoolNameController =
        TextEditingController(text: widget.survey.schoolName);
    placeController =
        TextEditingController(text: widget.survey.place.toString());
    schoolRankController =
        TextEditingController(text: widget.survey.schoolRank.toString());
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: PreferredSize(
        ...
        child: CustomAppBar(
            titleText: widget.survey.id == '' ? 'Add School' : 'Update School'),
      ),
      body: SingleChildScrollView(
        child: BlocConsumer<SurveyBloc, SurveyState>(
          listener: (BuildContext context, SurveyState state) {
            ...
            if (state.msg.contains(KString.suerveyUpdated) ||
                state.msg.contains(KString.surveyAdded)) {
                  ...
                  CustomButton(
                    ...
                    onTap: () {
                      final SurveyModel userProfileUpdate = SurveyModel(
                          ...
                          schoolRank:
                              int.parse(schoolRankController.text.trim()),
                          id: SurveyModel.generateUniqueId());

                      context
                          .read<SurveyBloc>()
                          .add(AddSurvey(body: userProfileUpdate));
                    },
                  ),
                ],
              ),
